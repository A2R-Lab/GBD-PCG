...
template <typename T>
uint32_t pcgSharedMemSize(uint32_t state_size, uint32_t knot_points){
...
template <typename T>
__global__
void pcg(const uint32_t state_size,
                 ...
                 const float exit_tol)
{
    ...
    __syncthreads();
    bdmv<T>(s_r_b, state_size, knot_points-1, s_S, s_lambda, block);
    __syncthreads();
    ...
    __syncthreads();
    bdmv<T>(s_r_tilde, state_size, knot_points-1, s_Pinv, s_r, block);
    __syncthreads();
    ...
    for(iter = 0; iter < max_iter; iter++){
        ...
        __syncthreads();
        bdmv<T>(s_upsilon, state_size, knot_points-1, s_S, s_p, block);
        __syncthreads();
        ...
        grid.sync(); //-------------------------------------
        glass::reduce(s_v_b, knot_points, d_v_temp, block);
        __syncthreads();
        alpha = eta / s_v_b[0];
        ...
        // r_tilde = Pinv * r
        loadbdVec<T>(s_r, state_size, knot_points, &d_r[block_x_statesize], block);
        __syncthreads();
        bdmv<T>(s_r_tilde, state_size, knot_points-1, s_Pinv, s_r, block);
        __syncthreads();
