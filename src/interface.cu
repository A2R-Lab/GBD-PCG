    ...
    pcg_config *config)
{
    ...
    void *pcg_kernel = (void *) pcg<float>;
    ...
    gpuErrchk(cudaLaunchCooperativeKernel(pcg_kernel, knot_points, config->pcg_block, kernelArgs, pcgSharedMemSize<T>(state_size, knot_points), streams[0]));
